...
public class InputBGenerator {

    DataSet dataSet;
    int buildingCounter = 0;

    public InputBGenerator() {
        dataSet = new DataSet();
    ...
    private Integer[] getPrices1(double base, int duration) {
        ...
        for (int i = 0; i < years; i++) {
            int approximatePrice = (int)(8 * Math.sin(0.05 * i - leftMove) + Math.sin(i) + base);
    ...
    private Integer[] getPrices2(double base, int duration) {
        ...
        for (int i = 0; i < years; i++) {
            int approximatePrice = (int)(80 * Math.sin(0.01 * i - leftMove) + Math.sin(i) + base);
            prices[i] = RandomGenerator.randomInteger(approximatePrice-5, approximatePrice+5);
        }
