...
public class InputCGenerator {

    DataSet dataSet;
    int buildingCounter = 0;

    public InputCGenerator() {
        dataSet = new DataSet();
    ...
    private Integer[] getPrices1(double base, int duration) {
        ...
        for (int i = 0; i < years; i++) {
            int approximatePrice = (int)(8 * Math.sin(0.05 * i - leftMove) + Math.sin(i) + base + 0.07*i);
    ...
    private Integer[] getPrices2(double base, int duration) {
        ...
        for (int i = 0; i < years; i++) {
            int approximatePrice = (int)(80 * Math.sin(0.01 * i - leftMove) + Math.sin(i) + base + 0.07*i);
            prices[i] = RandomGenerator.randomInteger(approximatePrice-5, approximatePrice+5);
        }
